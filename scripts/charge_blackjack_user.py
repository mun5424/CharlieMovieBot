#!/usr/bin/env python3
"""
Charge (debit) a user's blackjack balance by a given dollar amount.
...
charge is recorded in blackjack_transactions like any other balance change.
"""
...
async def charge_user(user_id: int, amount_cents: int, db_path: str = DB_FILE) -> None:
    db = BlackjackDB(db_path)
    await db.init()

    balance_before = await db.get_balance(user_id)
    balance_after = await db.add_balance(user_id, -amount_cents, "manual_admin_charge")

    print(f"User {user_id}")
    print(f"  Balance before: {money(balance_before)}")
    print(f"  Charged:        {money(amount_cents)}")
    print(f"  Balance after:  {money(balance_after)}")
    if balance_after < 0:
        print("  Warning: balance is now negative.")
...
def parse_amount_cents(amount_str: str) -> int:
    ...
    if dollars <= 0:
        raise SystemExit("Amount to charge must be greater than 0.")
...
def main() -> None:
    ...
    asyncio.run(charge_user(user_id, amount_cents))
